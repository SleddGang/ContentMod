    ...
    internal class FrozenHookProjectile : ModProjectile
    {
		...
		public override void AI()
		{
			...
			else
			{
				...
				Vector2 projectileLocation = new Vector2(projectile.position.X + projectile.width * 0.5f, projectile.position.Y + projectile.height * 0.5f);
				float xDistance = playerLocation.X - projectileLocation.X;
				float yDistance = playerLocation.Y - projectileLocation.Y;
				float grappleDistance = (float)Math.Sqrt(xDistance * xDistance + yDistance * yDistance);
				projectile.rotation = (float)Math.Atan2(yDistance, xDistance) - 1.57f;
				...
				if (projectile.ai[0] == 0f)
				{
					...
					for (int xPos = grappleLeft2; xPos < grappleRight2; xPos = newPosition + 1)
					{
						for (int yPos = grappleBottom2; yPos < grappleTop2; yPos = newPosition + 1)
						{
							...
							tilePosition.X = xPos * 16;
							tilePosition.Y = yPos * 16;
								...
								(Main.tileSolid[Main.tile[xPos, yPos].type] || Main.tile[xPos, yPos].type == 314))
							{
								...
								Main.PlaySound(SoundID.Dig, xPos * 16, yPos * 16, 1, 1f, 0f);
								...
								{
									NetMessage.SendData(MessageID.PlayerControls, -1, -1, null, projectile.owner, 0f, 0f, 0f, 0, 0, 0);
								}
								break;
							}
							newPosition = yPos;
				...
				else if (projectile.ai[0] == 2f)
				{
					...
					for (int xPos = grappleLeft; xPos < grappleRight; xPos = newPosition + 1)
					{
						for (int yPos = grappleBottom; yPos < grappleTop; yPos = newPosition + 1)
						{
							...
							tilePosition.X = xPos * 16;
							tilePosition.Y = yPos * 16;
							if (projectile.position.X + projectile.width / 2 > tilePosition.X &&
								projectile.position.X + projectile.width / 2 < tilePosition.X + 16f &&
								projectile.position.Y + projectile.height / 2 > tilePosition.Y &&
								projectile.position.Y + projectile.height / 2 < tilePosition.Y + 16f &&
								Main.tile[xPos, yPos].nactive() &&
